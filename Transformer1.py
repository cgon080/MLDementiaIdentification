...
class PositionalEncoding(nn.Module):
    def __init__(self, device):
        ...
        #self.seq_length = input_size_seq

        '''
        # Precompute the positional encodings for the maximum possible sequence length
        div_term = torch.exp(torch.arange(d_model, dtype=torch.float32) * -(math.log(10000.0) / d_model))

        pe = torch.zeros(seq_length, d_model)  # Create for the maximum possible sequence length
        ...
        # Register the positional encoding as a buffer (no gradients will be calculated for it)
        self.register_buffer('pe', pe)
        '''
    ...
    def forward(self, x, timestamps):
        ...
        # Expand the positional encodings for the batch size
        #position = (timestamps-timestamps.min()) / (timestamps.max()-timestamps.min())  # Shape: (batch_size, seq_length, 1)
        #div_term = torch.exp(torch.arange(self.d_model, dtype=torch.float32, device=timestamps.device) * -(math.log(10000.0) / self.d_model))
        ...
        # Compute new positional encoding based on timestamps
        #sin_values = torch.sin(position * div_term)
        #cos_values = torch.cos(position * div_term)

        #pe = torch.zeros(batch_size, self.seq_length, self.d_model, device=timestamps.device)
        #pe[:, :, 0::2] = sin_values[:, :, 0::2]
        #pe[:, :, 1::2] = cos_values[:, :, 1::2]
        
        # Repeat last features if needed
        #last_feature = pe[:, :, -1:]
        #repeated_features = last_feature.repeat(1, 1, 448)
        #pe = torch.cat([pe, repeated_features], dim=2)  # Assuming you still want to repeat features
        #pe = pe.transpose(2, 1)

        #if pe.shape != x.shape:
        #    raise ValueError(f"Positional encodings shape {pe.shape} must match input shape {x.shape}")
...
model1 = TransformerModel(input_size_code=input_size_code, hidden_size=hidden_size, output_size=output_size, static_size=static_size, input_size_feature = input_size_feature, input_size_seq = input_size_seq,
...
for epoch in range(num_epochs):
    ...
    for time_visit_batch, codes_batch, sequences_batch, timestamps_batch, static_batch, labels_batch in train_loader:
        ...
        optimizer.zero_grad()
        outputs = model1(codes_batch, sequences_batch, timestamps_batch, time_visit_batch, static_batch)
    ...
    with torch.no_grad():
        ...
        for time_visit_batch, codes_batch, sequences_batch, timestamps_batch, static_batch, labels_batch in val_loader:
            ...
            static_batch = static_batch.to(device)
            labels_batch = labels_batch.to(device)
            y_pred = model1(codes_batch, sequences_batch, timestamps_batch, time_visit_batch, static_batch)
...
torch.save(model1.state_dict(), r'D:\cgon080\ML\Data\Features1\Transf_96m.pth')
...
prediction, labels_pred_test = pred_test(test_loader=test_loader, model=model1) 
