...
embedding_dim = 100
num_classes = 1    
input_channels = sequences_train.shape[-2] + timestamps_train.shape[-2] + embedding_dim + 1
input_size_data =  static_train.shape[-1]   

model = CNNModel(input_channels=input_channels, num_classes=num_classes, embedding_dim=embedding_dim, input_size_data=input_size_data).to(device)  
model = model.double()
...
for epoch in range(num_epochs):
    ...
    for time_visit_batch, codes_batch, sequences_batch, timestamps_batch, static_batch, labels_batch in train_loader:
        ...
        optimizer.zero_grad()
        outputs = model(codes_batch, time_visit_batch.double(), sequences_batch.double(), timestamps_batch.double(), static_batch.double())
    ...
    with torch.no_grad():
        ...
        for time_visit_batch, codes_batch, sequences_batch, timestamps_batch, static_batch, labels_batch in val_loader:
            ...
            y_pred = model(codes_batch, time_visit_batch.double(), sequences_batch.double(), timestamps_batch.double(), static_batch.double()).squeeze()
            #y_pred = y_pred.float()
            #labels_batch = labels_batch.long()
...
plt.savefig(r'D:\cgon080\ML\DifferentTest\Time96m_retrain\loss_plot.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.savefig(r'D:\cgon080\ML\DifferentTest\Time96m_retrain\Acc_plot.png', dpi=300, bbox_inches='tight')
plt.close()   

torch.save(model.state_dict(), r'D:\cgon080\ML\DifferentTest\CNN_96.pth')
...
with torch.no_grad():
    ...
    for time_visit_batch, codes_batch, sequences_batch, timestamps_batch, static_batch, labels_batch in test_loader:
        ...
        y_pred = model(codes_batch, time_visit_batch, sequences_batch, timestamps_batch, static_batch)
